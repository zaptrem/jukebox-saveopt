...
class ConditionalAutoregressive2D(nn.Module):
    ...
    def __init__(self, input_shape, bins,
                 ...
                 encoder_dims=0, only_encode=False, merged_decoder=False, prime_len=None, device=t.device("mps")):
        super().__init__()
        self.input_shape = input_shape
        self.input_dims = input_dims = np.prod(input_shape)
    ...
    def get_emb(self, sample_t, n_samples, x, x_cond, y_cond):
        ...
        if sample_t == 0:
            # Fill in start token
            x = t.empty(n_samples, 1, self.width).to(t.device("mps"))
            if self.y_cond:
                x[:, 0] = y_cond.view(N, self.width)
            else:
                x[:, 0] = self.start_token
    ...
    def sample(self, n_samples, x_cond=None, y_cond=None, encoder_kv=None, fp16=False, temp=1.0, top_k=0, top_p=0.0,
        ...
        if self.x_cond:
            ...
            assert x_cond is None
            x_cond = t.zeros((N, 1, self.width), dtype=t.float).to(t.device("mps"))
    ...
    def check_sample(self, chunk_size):
        ...
        with t.no_grad():
            y_cond = t.randn(bs, 1, d).to(t.device("mps")) if self.y_cond else None
            x_cond = t.randn(bs, l, d).to(t.device("mps")) if self.x_cond else None
        ...
        for y_cond in [True, False]:
            for attn_order in [0,2,6,12]:
                prior = ConditionalAutoregressive2D(input_shape, bins,
                                                    ...
                                                    encoder_dims=encoder_dims, prime_len=prime_len).to(t.device("mps"))
