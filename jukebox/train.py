...
def log_inputs(orig_model, logger, x_in, y, x_out, hps, tag="train"):
    ...
    if hps.prior:
        if hps.labels:
            log_labels(logger, orig_model.labeller, f'{tag}_y_in', allgather(y.to(t.device("mps"))), hps)
    else:
...
def sample_prior(orig_model, ema, logger, x_in, y, hps):
    ...
    if hps.prior and hps.labels:
        log_labels(logger, orig_model.labeller, f'sample_x_T1', allgather(y.to(t.device("mps"))), hps)
...
def evaluate(model, orig_model, logger, metrics, data_processor, hps):
    ...
    with t.no_grad():
        for i, x in logger.get_range(data_processor.test_loader):
            ...
            if y is not None:
                y = y.to(t.device("mps"), non_blocking=True)
