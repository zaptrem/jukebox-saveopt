...
class SyncBatchNorm(_BatchNorm):
    ...
    Examples::
        ...
        >>> sbn = apex.parallel.SyncBatchNorm(100).cpu()
        >>> inp = torch.randn(10, 100, 14, 14).cpu()
        >>> out = sbn(inp)
        >>> inp = torch.randn(3, 100, 20).cpu()
        >>> out = sbn(inp)
        >>> # channel last tensor
        >>> sbn = apex.parallel.SyncBatchNorm(100, channel_last=True).cpu()
        >>> inp = torch.randn(10, 14, 14, 100).cpu()
    """
